function updateImage() {
  document.getElementById("demo").src = document.getElementById("backgroundImage").value
}

function updateBcColor() {
  document.getElementById("bcColorDemo").style.backgroundColor = document.getElementById("backgroundColor").value
}
...
function newOption(questionNumber) {
  var optionsDiv = document.getElementById("question" + questionNumber + "Options");
  var optionCounter = 0;
  document.querySelectorAll(".question" + questionNumber + "Option").forEach(function(option) {
    optionCounter ++
  });
  ...
  if (optionCounter==1) {
    ...
    document.getElementById("questionsDiv").append(button, document.createElement("BR"))
  }
}
...
async function createQuiz() {
  ...
  var bodyOfRequest = {quizName: document.getElementById("quizName").value, questions: [], backgroundColor: document.getElementById("backgroundColor").value, backgroundImage: document.getElementById("backgroundImage").value}
  for (var i = 1;i<questionCounter;i++) {
    ...
    var optionCounter = 1;
    var optionArray = [];
    document.querySelectorAll(".question" + i + "OptionTextbox").forEach(function(textbox) {
      optionArray.push(document.getElementById("question" + i + "option" + optionCounter).value)
      optionCounter++
    });
    ...
    var correctOption = document.querySelector("input[name=question" + i + "]:checked").value;
  ...
  var response  = await fetch("/createQuiz", {method: "POST",body: JSON.stringify(bodyOfRequest),headers: {"Content-Type": "application/json"}});
