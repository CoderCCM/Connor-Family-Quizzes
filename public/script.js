...
quizList.forEach(function(item, index) {
  ...
  document.getElementById("quizList").appendChild(document.createElement("br"))
  document.getElementById("quizList").appendChild(document.createElement("br"))
  document.getElementById("quizList").appendChild(document.createElement("br"))
})
...
async function openScorePage(quizName) {
  ...
  document.getElementById("container").style.backgroundColor = data.backgroundColor
...
actualQuestions = data.questions;
  ...
  var response  = await fetch("/requestChoicesByName", {method: "POST",body: JSON.stringify({quizName: quizName, participantName: localStorage.getItem("name")}),headers: {"Content-Type": "application/json"}});
var data = await response.json();
var choices = data.choices;
  ...
  h2.id = "scoreH2"
    container.append(h2);
  var h2 = document.createElement("h2");
    h2.innerHTML = "Attempted: " + new Date(Number(localStorage.getItem(quizName + "-Taken")))
    c.append(h2);
  ...
  document.getElementById("scoreH2").innerHTML = localStorage.getItem("name") + "'s score: " + String(correctQuestionCounter + '/' + actualQuestions.length + ' ' + String(Math.floor(Number(correctQuestionCounter/actualQuestions.length)*100)) + '%');
  ...
  async function beginQuiz(qName) {
    ...
    while (!allowedToPass) {
      promptForName()
      var response  = await fetch("/requestIfNameAlreadyExists", {method: "POST",body: JSON.stringify({participantName: name, quiz: qName}),headers: {"Content-Type": "application/json"}});
      var data = await response.json();
    ...
    var response  = await fetch("/requestQuestions", {method: "POST",body: JSON.stringify({quizName: qName}),headers: {"Content-Type": "application/json"}});
var data = await response.json();
questions = data.questions;
  ...
  function createQuestionElement(index) {
    ...
    var header = $('<h2>Question ' + (index + 1) + '/' + questions.length + ' (' + Math.round(Number((index)/questions.length) * 100) + '%  completed):</h2>');
  ...
  async function displayScore() {
    ...
    var score = $('<h2>',{id: 'question'});
        ...
        var response  = await fetch("/submitChoicesByName", {method: "POST",body: JSON.stringify({quizName: quizName, participantName: localStorage.getItem("name"), choices: selections}),headers: {"Content-Type": "application/json"}});
      var data = await response.json();
    ...
    var h2 = document.createElement("h2");
    h2.innerHTML = localStorage.getItem("name") + "'s score: " + numCorrect + '/' + questions.length + ' ' + String(Math.floor(Number(numCorrect/questions.length)*100)) + '%</h2>'
    c.append(h2);
    var h2 = document.createElement("h2");
    h2.innerHTML = "Attempted: " + new Date(Number(localStorage.getItem(quizName + "-Taken")))
    c.append(h2);
