...
async function blah() {  
var response  = await fetch("/requestQuizList", {method: "POST",body: JSON.stringify({a: "A"}),headers: {"Content-Type": "application/json"}});
...
var quizList = data.quizList
       ...
       quizList.forEach(function(item) {
           ...
           document.body.append(button, document.createElement("BR"), document.createElement("BR"), document.createElement("BR"));
       })
}

   async function openQuizOverview(quizName) {
     var response  = await fetch("/requestQuestions", {method: "POST",body: JSON.stringify({quizName: quizName}),headers: {"Content-Type": "application/json"}});
  ...
  var data = await response.json();
   ...
   async function paintColumnOne(quizName) {
    ...
    var scoreArray = [];
     ...
     var response  = await fetch("/requestParticipantListByQuizName", {method: "POST",body: JSON.stringify({quizName: quizName}),headers: {"Content-Type": "application/json"}});
  console.log(response)
  var data = await response.json();
            ...
            document.getElementById("participantCount").innerHTML = "Participants: " + String(Number(a.length));
            var highestscore = 0, highestIndex = [];
            a.forEach(async function(item, indexier) {
                ...
                var p = document.createElement("p");
                 ...
                 var response  = await fetch("/requestChoicesByName", {method: "POST",body: JSON.stringify({quizName: quizName, participantName: item}),headers: {"Content-Type": "application/json"}}).then(async response =>{
                ...
                var data = await response.json().then(async data =>{
                    ...
                    var score = Math.floor(Number(numRight/quizzes[quizName].questions.length)*100);
                    scoreArray.push(Math.floor(Number(numRight/quizzes[quizName].questions.length)*100));
            ...
            console.log(indexier)
                ...
                document.getElementById("participantCount").innerHTML = "Participants: " + Number(a.length) + ", avg. score: " + average + "%";
   ...
   async function openQuestionOverview() {  
     ...
     document.getElementById("questionOverviewOverlay").style.backgroundImage = "url(" + bcImage + ")"
       selections = [];
     
       var response  = await fetch("/requestParticipantListByQuizName", {method: "POST",body: JSON.stringify({quizName: quizNameGlobal}),headers: {"Content-Type": "application/json"}});
  console.log(response)
  var data = await response.json();
           ...
           f.forEach(async function(quizTaker, index) {
             console.log("QT: " + quizTaker)
               if (quizTaker!=="QWERTYUIOP") {
                 ...
                 var response  = await fetch("/requestChoicesByName", {method: "POST",body: JSON.stringify({quizName: quizNameGlobal, participantName: quizTaker}),headers: {"Content-Type": "application/json"}}).then(async responsee =>{
       ...
       e.innerHTML = "";
       quizzes[quizNameGlobal].questions.forEach(function(question, indexer) {
            var h2 = document.createElement("h2");
            h2.innerHTML = Number(indexer+1) + ". " + question.question;
            e.append(h2);
            question.choices.forEach(function(choice, index) {
                ...
                h3.innerHTML = findNumPeopleThatChoseOption(selections, indexer,index) + "~~~~~~~~~" + choice;
                if (index==question.correctAnswer) {
                    h3.style.backgroundColor = "lightgreen";
                }
                e.append(h3);
       ...
       document.getElementById("questionOverviewOverlay").style.display = "block";
                 
                 
                 document.getElementById("questionOverviewOpener").style.display = "inline-block";
               }
   ...
   function findNumPeopleThatChoseOption(selectionsa, indexer, index) {
    ...
    var keysArray = [];
                ...
                return String(numPeoplePicked + " (" +  Math.round( Number(Number(numPeoplePicked/selectionsa.length)*100) * 10 ) / 10+ "%)");
   }

function openLiveScoreboard() {
  location.replace("https://connorsfamilyquizzes.replit.app/scoreboard?qName=" + encodeURIComponent(quizNameGlobal));
}