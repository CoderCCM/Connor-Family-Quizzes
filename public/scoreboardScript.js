...
function blah() {
  ...
  if(qName) {
    ...
    const qrcode = new QRCode(document.getElementById('QR'), {
      text: 'https://connorsfamilyquizzes.replit.app?qNameAutoOpen=' + encodeURIComponent(qName),
      width: 256,
      height: 256,
      colorDark : '#000',
      colorLight : '#fff',
  ...
  var data = await response.json();
   ...
   async function paintColumnOne(quizName) {
     ...
     var response  = await fetch("/requestParticipantListByQuizNameLive", {method: "POST",body: JSON.stringify({quizName: quizName}),headers: {"Content-Type": "application/json"}});
  console.log(response)
  var data = await response.json();
  ...
  var a = data.participants;
            ...
            a.forEach(async function(item, indexier) {
                ...
                var p = document.createElement("p");
                 ...
                 var response  = await fetch("/requestChoicesByName", {method: "POST",body: JSON.stringify({quizName: quizNameGlobal, participantName: item}),headers: {"Content-Type": "application/json"}}).then(async response =>{
                ...
                var data = await response.json().then(async data =>{
                    ...
                    b.forEach(function(selection, index) {
                        if (selection==quizzes[quizNameGlobal].questions[index].correctAnswer) {
                            numRight++;
                        }
                    });
                    ...
                    var score = Math.floor(Number(numRight/quizzes[quizNameGlobal].questions.length)*100);
                    scoreArray.push(Math.floor(Number(numRight/quizzes[quizNameGlobal].questions.length)*100));
            ...
            console.log(indexier)
                ...
                document.getElementById("participantCount").innerHTML = "Participants: " + Number(a.length);
                    document.getElementById("avgScore").innerHTML = "Current Average: " + average + "%";
...
async function repeatedFileRead() {
  var r  = await fetch("/readLiveStreamFile", {method: "POST",body: JSON.stringify({fn: liveStreamDataFileName}),headers: {"Content-Type": "application/json"}});
  ...
  document.getElementById("updateTime").innerHTML = "Last Updated: " + new Date().toLocaleTimeString();
}